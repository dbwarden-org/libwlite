 ...
 *
 *   file        := top_level*
 *   top_level   := model_decl | index_decl | view_decl | trigger_decl | database_decl
 *
 ...
 *                    | 'unique' '(' cols ')'
 *                    | 'check' '(' EXPR ')'
 *                    | 'foreign_key' '(' cols ')' 'references' IDENT '(' cols ')'
 *
 ...
 *   trigger_decl := 'trigger' IDENT '{' ... '}'
 *   database_decl := 'database' '{' pragma* '}'
 */
...
static Token next_token(Lexer *l) {
    ...
    switch (c) {
        ...
        case '\'': case '"': {
            ...
            if (*l->current) l->current++;
            t.type = TOK_STRING;
            t.length = (size_t)(l->current - t.start);
            return t;
        }
        default:
            if (isdigit(c)) {
...
static char *expect_ident(Lexer *l, wlite_error **err) {
    ...
    memcpy(s, t.start, t.length); s[t.length] = '\0';
    return s;
}
...
/* consume tokens until RBRACE or EOF (for body blocks) */
...
/* consume expression until next keyword-level token */
...
static int parse_field(Lexer *l, WlTable *table, wlite_error **err) {
    /* The 'field' keyword is consumed by match_ident_ci in the model loop.
       If we get here, the next token is the field name (after 'field' was consumed). */
    WlColumn col = {0};
    ...
    if (type_tok.type != TOK_IDENT) {
        if (err) { (*err)->code = WLITE_ERR_SYNTAX; (*err)->message = strdup("expected field type");
            (*err)->line = type_tok.line; }
        return 0;
    }
        ...
        if (t.type == TOK_LBRACE) {
            ...
            while (peek_token(l).type != TOK_RBRACE && peek_token(l).type != TOK_EOF) {
                ...
                if (match_ident_ci(l, "primary_key")) col.primary_key = 1;
                    ...
                    if (expect_token(l, TOK_LPAREN, err)) {
                        col.generated_expr = consume_balanced(l);
                        col.is_generated = 1;
                    }
                    ...
                    next_token(l); /* skip unknown attribute */
                }
            }
            if (peek_token(l).type == TOK_RBRACE) next_token(l); /* consume } */
            continue;
        }
        if (t.type != TOK_IDENT) break;
...
static int parse_model(Lexer *l, WlSchema *schema, wlite_error **err) {
    ...
    while (peek_token(l).type != TOK_RBRACE && peek_token(l).type != TOK_EOF) {
        /* model options */
        if (match_ident_ci(l, "table")) {
            table.name = expect_string(l, err);  /* override name from table "..." */
            continue;
        }
        if (match_ident_ci(l, "strict")) { table.strict = 1; continue; }
        if (match_ident_ci(l, "without")) {
            if (match_ident_ci(l, "rowid")) table.without_rowid = 1;
            continue;
        }
        if (match_ident_ci(l, "comment")) {
            table.comment = expect_string(l, err);
            continue;
        }
            ...
            if (expect_token(l, TOK_LPAREN, err)) {
                while (peek_token(l).type != TOK_RPAREN) {
                    ...
                    if (col) {
                        ...
                        table.primary_key.columns[table.primary_key.column_count++] = col;
                    }
            ...
            if (expect_token(l, TOK_LPAREN, err)) {
                ...
                while (peek_token(l).type != TOK_RPAREN) {
                    ...
                    if (col) {
                        ...
                        uq.columns[uq.column_count++] = col;
                    }
                    if (peek_token(l).type == TOK_COMMA) next_token(l);
                }
                expect_token(l, TOK_RPAREN, err);
        ...
        if (match_ident_ci(l, "foreign_key")) {            if (expect_token(l, TOK_LPAREN, err)) {
                ...
                while (peek_token(l).type != TOK_RPAREN) {
                    ...
                    if (col) {
                        ...
                        fk.columns[fk.column_count++] = col;
                    }
                    if (peek_token(l).type == TOK_COMMA) next_token(l);
                }
                expect_token(l, TOK_RPAREN, err);
                        ...
                        while (peek_token(l).type != TOK_RPAREN) {
                            ...
                            if (col) {
                                fk.ref_columns = realloc(fk.ref_columns,
                                    (fk.ref_column_count + 1) * sizeof(char *));
                                fk.ref_columns[fk.ref_column_count++] = col;
                            }
                            if (peek_token(l).type == TOK_COMMA) next_token(l);
                        }
                        expect_token(l, TOK_RPAREN, err);
        ...
        if (match_ident_ci(l, "field")) {
            ...
            /* unknown token in model body — skip */
            next_token(l);
        }
    }
...
static int parse_index(Lexer *l, WlSchema *schema, wlite_error **err) {
    WlIndex idx = {0};
    idx.name = expect_ident(l, err);
    if (!idx.name) return 0;
    if (!expect_token(l, TOK_LBRACE, err)) return 0;
...
static int parse_view(Lexer *l, WlSchema *schema, wlite_error **err) {
    WlView view = {0};
    view.name = expect_ident(l, err);
    if (!view.name) return 0;
    if (!expect_token(l, TOK_LBRACE, err)) return 0;
    if (match_ident_ci(l, "select")) {
        ...
        view.sql = malloc(sql_len + 1);
        memcpy(view.sql, sql_start, sql_len); view.sql[sql_len] = '\0';
    }
    expect_token(l, TOK_RBRACE, err);
...
static int parse_trigger(Lexer *l, WlSchema *schema, wlite_error **err) {
    WlTrigger trig = {0};
    trig.name = expect_ident(l, err);
    if (!trig.name) return 0;
    if (!expect_token(l, TOK_LBRACE, err)) return 0;
    /* skip timing */
    ...
    if (match_ident_ci(l, "begin")) {
        ...
        while (*l->current) {
            ...
            if (depth == 0 && *l->current == 'e' && strncmp(l->current, "end", 3) == 0 && !is_alnum(l->current[3])) {
                l->current += 3; break;
            }
            l->current++;
        }
        ...
        trig.sql = malloc(body_len + 1);
        memcpy(trig.sql, body_start, body_len); trig.sql[body_len] = '\0';
    }
    expect_token(l, TOK_RBRACE, err);
...
static int parse_model_config(Lexer *l, WlSchema *schema, wlite_error **err) {
    ...
    while (peek_token(l).type != TOK_RBRACE && peek_token(l).type != TOK_EOF) {
        ...
        } else if (match_ident_ci(l, "version")) {
            Token t = next_token(l);
            if (t.type == TOK_NUMBER) {
                schema->model_version = atoi(t.start);
            }
        } else {
...
WlSchema *wl_schema_parse(const char *source, size_t length, wlite_error **error) {
    ...
    memcpy(buf, source, length); buf[length] = '\0';
    ...
    while (peek_token(&lexer).type != TOK_EOF) {
        if (!parse_top_level(&lexer, schema, error)) {
